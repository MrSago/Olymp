...
fun main() {
    ...
    while (l <= r) {
        var okLeft = false
        var okRight = false
        if (last < a[l]) okLeft = true
        if (last < a[r]) okRight = true
        if (!okLeft && !okRight) break
        if (okLeft && okRight && a[l] == a[r]) {
            var cl = 1
            var cr = 1
            while (l + cl <= r && a[l + cl] > a[l + cl - 1]) ++cl
            while (r - cr >= l && a[r - cr] > a[r - cr + 1]) ++cr
            if (cl > cr) {
                for (i in count until count + cl) res[i] = 'L'
                count += cl
            } else {
                for (i in count until count + cr) res[i] = 'R'
                count += cr
            }
            break
        }
    ...
    println(count)
    println(res.slice(0 until count).joinToString(""))
}

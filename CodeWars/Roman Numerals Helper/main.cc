...
class RomanHelper {
 private:
  map<char, int> alph = {{'I', 1},   {'V', 5},   {'X', 10},  {'L', 50},
                         {'C', 100}, {'D', 500}, {'M', 1000}};
  map<int, string> table = {
      {1, "I"},     {10, "X"},     {100, "C"},   {1000, "M"},  {2, "II"},
      {20, "XX"},   {200, "CC"},   {2000, "MM"}, {3, "III"},   {30, "XXX"},
      {300, "CCC"}, {3000, "MMM"}, {4, "IV"},    {40, "XL"},   {400, "CD"},
      {5, "V"},     {50, "L"},     {500, "D"},   {6, "VI"},    {60, "LX"},
      {600, "DC"},  {7, "VII"},    {70, "LXX"},  {700, "DCC"}, {8, "VIII"},
      {80, "LXXX"}, {800, "DCCC"}, {9, "IX"},    {90, "XC"},   {900, "CM"}};
...
void full_test() {
  ...
  cout << "\n=====================TEST=====================\n";
  cout << "Input" << setw(fix + 1) << right << "Result" << setw(fix) << right
       << "Verdict\n";
  for (int n = 1; n <= MAXN; ++n) {
    ...
    cout << "n=" << n << setw(fix) << right << roman << '=' << arab << setw(fix)
         << right << (arab == n ? "ACCEPTED" : "FAILED") << '\n';
  }
  cout << "==============================================\n";

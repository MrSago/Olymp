...
inline void debugInFile() { assert(freopen("debug.out", "w", stdout)); }
...
inline void castleVis(std::ifstream& fin) {
  ...
  int tmp;
  fin >> tmp >> tmp;
  std::vector<std::vector<char>> field(N + N - 1,
                                       std::vector<char>(M + M - 1, '#'));
...
int main() {
  ...
  for (int i = 0; i < (int)components.size() - 1; ++i) {
    for (int j = i + 1; j < (int)components.size(); ++j) {
      for (auto& k : components[i]) {
        ...
        for (auto& s : v) {
          if (s != -1 && find(components[j].begin(), components[j].end(), s) !=
                             components[j].end()) {
            ...
            if (sz > res) {
              ...
              if (k % M < connection.first % M) {
                connection = {k, s};
              } else if ((k % M == connection.first % M) &&
                         (k / M > connection.first / M)) {
                connection = {k, s};
              }
            }
  ...
  fout << components.size() << '\n'
       << components[0].size() << '\n'
       << res << '\n'
       << connection.first / M + 1 << ' ' << connection.first % M + 1 << ' '
       << dir << '\n';
